...
{ 
	...
	{				
		...
		if ($pos === FALSE)
		{
			...
			/*
			if (esCampoInt($tipoCampo) && ($i <= 2)) //tipo INT, prueba si es una FK
			{
				$valorCampo = getFKValue2($nombreCampo, $valorCampo, $qinfo->getFieldsRef(), $fk_cache, false);
			}
			*/
...
/*				if (($i <= 2) && ($info == 0))
				{
					echo("<a href=\"javascript:selRec('" . $rcontrol1 . "', '" . $rcontrol2 . "', '" . $valorKey ."', '" . $valorDesc ."');\">" . $valorCampo . "</a>");
				}
				else	
*/				
