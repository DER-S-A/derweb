...
$rsChk = getRs("select table_checksum
				from sc_querys 
				where table_ = '$tabla'");
$checksum = $rsChk->getValueInt("table_checksum");
$rsChk->close();
...
//incluye los parámetros pero obliga a refrezcar 6' minutos al cambar el etag
$cuarto = intdiv(date("i"), 10);
$etag = md5($params) . date("-H-$cuarto-$checksum");
...
{ 
		debug("CACHE: sin cambios Etag: $etag");
...
$rfilterfield = Request("filterfield");
$rcondicion = Request("condicion");
...
if (!sonIguales($savedSql, ""))
{
	...
	$sql = $qinfo->getQuerySql2($rfilterfield, $rpalabra, $rorderby, "", $rcondicion, $rmquery, $rmid, $mfield, $filter, "", false);
...
<div class="">
	...
	<header class="w3-container headerTitulo">
		...
		{
		?>
			<a class="w3-button w3-text-white w3-right boton-fa-sup w3-hide-small" href="sc-escritorioagregar.php?query=<?php echo($rquery . "&filter=" . $filter . "&filtername=" . $filtername); ?>" target="contenido" title="Agregar o quitar a Favoritos">
		...
		?>
			...
			<a class="w3-button w3-text-white w3-right boton-fa-sup w3-hide-small" title="Exportar a Excel" href="<?php echo($urlcsv); ?>" target="_blank"> 
				<i class="fa fa-file-excel-o " aria-hidden="true"></i>
			</a>

			<a class="w3-button w3-text-white w3-right boton-fa-sup" title="Exportar a pdf / imprimir" href="<?php echo($urlpdf); ?>" target="_blank"> 
...
<div class="w3-card w3-light-gray w3-row">
	...
	<div class="w3-halfX">
		...
		$c = new HtmlCombo("condicion", $rcondicion);
		$c->add("CON", "contiene");
		$c->add("IGU", "igual");
		$c->add("DIF", "distinto");
		$c->add("MAY", "mayor");
		$c->add("MEN", "menor");
		$c->add("COM", "comienza");
		$c->setClass("w3-margin-small w3-right $hideMedium w3-hide-small");
		echo($c->toHtml());
		
		echo(getComboCamposArray($qinfo, "filterfield", " - campo - ", $rfilterfield, "w3-margin-small w3-right $hideMedium w3-hide-small"));
		?>
	</div>
