...
function linkEditar($xregistrovalor, $xid = "")
{
	...
	if (!sonIguales($xid, ""))
		$titulo .= " [F2]";
	$str .= "<i class=\"fa fa-pencil-square-o fa-2x\" title=\"Editar\"></i>";
	$str .= "</a>";
	return $str;
}
...
function linkBorrar($xregistrovalor, $xflat = false)
{
	$str = "<a class=\"w3-button w3-text-purple boton-fa\" href=\"javascript:dardebaja(" . $xregistrovalor . ")\">
				<i class=\"fa fa-trash-o fa-2x\" title=\"Borrar\"></i>
			</a>";
	return $str;
}
...
<script language="javascript">
	function dardebaja(xrecid)
	{
		if (confirm("Esta seguro que desea borrar este registro ?"))
			document.location.href = "delitem.php<?php echo(getParams()) ?>" + "&registrovalor=" + xrecid
	}
      ...
      <?php 
		...
		while ($i < $rsPpal->cantF())
		{
			...
			if (!sonIguales($mfield, $fieldname) && ($pos === FALSE))
			{
				...
				echo(" class=\"grid_header2 ");
				echo(getDataSorter($tipoCampo));
				echo("\"><b>");
				echo($qinfo->getFieldCaption($fieldname));
				echo("</b></th>");
				
				$rowFilters .= "<th class=\"grid_filter\" align=\"center\">";
		...
		echo("<th align=\"center\" width=\"50\" colspan=\"3\" class=\"grid_header2\">");
...
{ 
	...
	{				
		...
		if (!sonIguales($mfield, $nombreCampo) && ($pos === FALSE))
		{
			...
			echo("<td valign=\"top\" align=\"" . $dataAlign . "\" class=\"" . $qinfo->getFieldClass($nombreCampo, $record) . "\">");
			...
			else
			{
				...
				if ($qinfo->isFileField($nombreCampo))
					echo(sc3getImgSmall(getImagesPath(), $valorCampo)); 
				else
	...
	echo("<td class=\"td_toolbar_small\">");
	...
	echo("</td><td class=\"td_toolbar_small\">");
