...
document.onkeyup = function (e) {
	...
	*/
	...
	//analiza si presion� teclado num�rico
	if (kC >= 96 && kC <= 105)
		num = kC - 96;
...
function sumSetStatus(input) {
	if (document.getElementById('sumtotal') != null) {
		...
		document.getElementById('sumtotal').style.background = "#99FF99";
	}
}
...
function sumResetStatus() {
	if (document.getElementById('sumtotal') != null) {
		...
		document.getElementById('sumtotal').style.background = "#eee";
	}
}
...
function sumStart(e, obj) {
	...
	if (evt.ctrlKey) {
		...
		suma = Math.round(suma * 100) / 100;
		sumando++;
		statusText = "<img src='images/sum.png' border='0' height='12'/> = <b>" + suma + "</b> (" + sumando + ")";
		sumSetSumado(obj);
