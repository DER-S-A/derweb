...
if (getParameterInt("sc3-phpids", "0") || $FORCE_PHPIDS)
{
	// set the include path properly for PHPIDS
	set_include_path(get_include_path() . PATH_SEPARATOR . './terceros/phpids-0.7/lib/');
	...
	try {
	    ...
	    */
	    ...
	    $init = IDS_Init::init(dirname(__FILE__) . '/terceros/phpids-0.7/lib/IDS/Config/Config.ini.php');
	    ...
	    $init->config['General']['base_path'] = dirname(__FILE__) . '/terceros/phpids-0.7/lib/IDS/';
	    ...
	    */
	    if (!$result->isEmpty()) 
		{
	        ...
	        */
	        ...
	        //echo $result;
	        header("Location:./sc-error.php?code=ids&msg=");
			exit; 
