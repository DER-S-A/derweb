...
if (enviado()) {
    ...
    $notas = RequestStr("txt_notas");
    $objModel = new ClientesPotencialesModel();
    $objModel->agregarNota($mid, $notas);
...
<html>
    ...
    <body>
        ...
        <?php 
			...
			$req->add('sel_cliente_potencial', 'Cliente potencial');
			$cpo_sel_cliente_potencial = new HtmlEtiquetaValor('Cliente potencial', $input_sel_cliente_potencial->toHtml());
			$input_txt_notas = new HtmlInputTextarea("txt_notas", '');
			$cpo_txt_notas = new HtmlEtiquetaValor('Notas', $input_txt_notas->toHtml());
			$tab->agregarSolapa("Datos", '', $cpo_sel_cliente_potencial->toHtml() . $cpo_txt_notas->toHtml() );

        ?>   

        <form id="form1" action="der-clipot-registrar-notas.php" method="post" id="form1" name="form1">
