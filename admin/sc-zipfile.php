...
/**
 * Retorna un nombre de archivo temporal en la carpeta backups/ con el ZIP y un nombre dado
 * @param string $xname Si es vacio toma el nombre del sistema
 * @return string
 */
function getZipFilename($xname = "", $xtime = true)
{
	global $SITIO;
	$file = $xname;
	if (esVacio($xname))
		$file = $SITIO;
	$file .= "-" . Sc3FechaUtils::formatFecha3($xtime);
	if ($xtime)
		$file .= "-" . getClave(5);
	$file .= ".zip";
...
if (!sonIguales($content, "") && !sonIguales($zip, ""))
{
	$zip = str_replace("/", "", str_replace("./", "", $zip));
	
	$zipfile = new zipfile();
	$zipfile->add_file(implode("", file($content)), str_replace("/", "", str_replace("./", "", $content)));
