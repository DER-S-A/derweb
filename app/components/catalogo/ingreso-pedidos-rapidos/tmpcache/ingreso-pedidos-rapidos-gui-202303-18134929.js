...
class IngresoPedidosRapidoGUI extends ComponentManager {
     ...
     */
    acomodarFooter() {
        if (this.__objDataGrid.row.count() !== 0) {
    ...
    __agregarItem() {        
        ...
        fila = [
            this.__nroRenglon, 
            articulo.values[0].codigo, 
            articulo.values[0].desc, 
            parseFloat(document.getElementById("txtCantidad").value),
            parseFloat(articulo.values[0].cped).toFixed(2),
            (articulo.values[0].cped * parseInt(document.getElementById("txtCantidad").value)).toFixed(2)
        ];
