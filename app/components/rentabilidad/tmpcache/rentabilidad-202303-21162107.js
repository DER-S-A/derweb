class Rentabilidad extends ComponentManager {
    ...
    cargarTabla(marcas, rubros, subrubros) {
        ...
        quitarBoton.addEventListener('click', () => {
            ...
            this.__repintarTabla(objTabla, index, dataTableRenta, objTablaEliminar, marcas, rubros, subrubros);
    ...
    __repintarTabla(objTabla, index, dataTableRenta, objTablaEliminar, marcas, rubros, subrubros) {
        dataTableRenta.clear().draw();
        objTablaEliminar.push(objTabla.splice(index, 1));
        console.log(marcas)
        console.log(objTabla);
        objTabla.map(tabla => {
            let xMarca = marcas.find( marca => tabla.marca == marca.id);
            console.log(xMarca.descripcion);
            let xrubro = rubros.find( rubro => tabla.rubro == rubro.id);
            let xsubrubro = subrubros.find( subrubro => tabla.subrubro == subrubro.id);
            dataTableRenta.row.add([xMarca.descripcion, xrubro.descripcion, xsubrubro.descripcion, margen1.value, margen2.value]);
            dataTableRenta.draw();
        });
    }
