class Rentabilidad extends ComponentManager {
    ...
    __repintarTabla(objTabla, index, dataTableRenta, objTablaEliminar, marcas, rubros, subrubros) {
        ...
        console.log(objTabla);
        /*objTabla.map(tabla => {
            let xMarca = marcas.find( marca => objTabla.marca == marca.id);
            let xrubro = rubros.find( rubro => objTabla.rubro == rubro.id);
            let xsubrubro = subrubros.find( subrubro => objTabla.subrubro == subrubro.id);
