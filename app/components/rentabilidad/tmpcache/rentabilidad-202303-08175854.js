class Rentabilidad extends ComponentManager {
    ...
    __llenarBox(listado, query_selector, value = 'TODAS') {
        ...
        if(value == 'TODAS') objSelector.append(`<option value="TODAS">Rubros</option>`);
    ...
    __llenarBoxFiltrado(query_selector, marcas, rubros, subrubros) {
        ...
        objMarcas.addEventListener('change', () => {
            ...
            this.__filtrarBoxes(consulta, objMarcas, objRubros, objSubrubros);
            
        })
    }

    __filtrarBoxes(consulta, objMarcas, objRubros, objSubrubros) {
...
function filtrarNumerosArray(listado, clave) {
    ...
    let resultado = [];
    let objeto = {};
    for(let i=0; i<lista.length<i;i++) {
        console.log(lista[i]);
        console.log(clave);
        resultado.push({clave:lista[i]});
    }
    return resultado;
}