class Rentabilidad extends ComponentManager {
    ...
    __llenarBoxFiltrado(query_selector, marcas, rubros, subrubros) {
        ...
        objMarcas.addEventListener('change', () => {
            ...
            this.__filtrarBoxesParaMarcas(consulta, objMarcas, objRubros, objSubrubros, rubros, subrubros);
    ...
    __filtrarBoxesParaMarcas(consulta, objMarcas, objRubros, objSubrubros, rubros, subrubros) {
    ...
    __filtrarBoxesParaRubros(consulta, objMarcas, objRubros, objSubrubros, marcas, subrubros) {
        ...
        if(objSubrubros.value == 'TODAS') {
            new APIs().call(url, parametros, 'POST', xdatos => {
                ...
                let listado = filtrarNumerosArray(xdatos,'id_subrubro');
                let marcaSeleccionada = objSubrubros.value;
                objSubrubros.innerHtml = '';
                console.log(listado);
                this.__llenarBox(filtrarArray(listado, subrubros), objSubrubros, marcaSeleccionada);
            }); 
            parametros = `where=id_rubro=${objRubros.value}`;
            new APIs().call(url, parametros, 'POST', xdatos => {
            console.log(xdatos);
            let listado = filtrarNumerosArray(xdatos,'id_marca');
            let subrubroSeleccionada = objMarcas.value;
            objMarcas.innerHTML = '';
            console.log(listado);
            this.__llenarBox(filtrarArray(listado, marcas), objMarcas, subrubroSeleccionada);
            });
        } else {
    ...
    __filtrarBoxesParaSubrubros(consulta, objMarcas, objRubros, objSubrubros, marcas, rubros) {
        ...
        if(objRubros.value == 'TODAS' &&  objMarcas.value == 'TODAS') {
    ...
    __crearConsultaParaFiltro2(objMarcas, objRubros, objSubrubros, opcion) {
        let consulta = '';

        if(objSubrubros.value == 'TODAS' && objMarcas.value == 'TODAS') consulta += `id_rubro=${objRubros.value}`;
    ...
    __crearConsultaParaFiltro(objMarcas, objRubros, objSubrubros, opcion) {
        ...
        switch(opcion) {
            ...
            break;
            case 'rubros': {
                //if(objRubros.value == 'TODAS' || objMarcas.value == 'TODAS') consulta += `id_rubro=${objRubros.value}`;
