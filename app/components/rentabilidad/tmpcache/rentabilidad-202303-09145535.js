class Rentabilidad extends ComponentManager {
     ...
     */
    __llenarBox(listado, query_selector, value = 'TODAS', eleccion = false) {
        ...
        listado.forEach(lista => {
            ...
            if(eleccion) {
                for(let i=0;i<objSelector.options.length;i++) {
                    if(objSelector.options[i].value == eleccion) objSelector.selectedIndex = i;
                    break;
                }
            }
        })
    ...
    __llenarBoxFiltrado(query_selector, marcas, rubros, subrubros) {
        ...
        objRubros.addEventListener('change', () => {
            ...
            this.__filtrarBoxesParaRubros(consulta, objMarcas, objSubrubros, marcas, subrubros);
    ...
    __filtrarBoxesParaMarcas(consulta, objRubros, objSubrubros, rubros, subrubros) {
        ...
        new APIs().call(url, parametros, 'POST', xdatos => {
            ...
            objRubros.innerHTML = '';
            this.__llenarBox(filtrarArray(listado,rubros), objRubros, rubroSeleccionado, rubroSeleccionado);
            listado = filtrarNumerosArray(xdatos,'id_subrubro');
            let subrubroSeleccionado = objSubrubros.value;
            objSubrubros.innerHtml = '';
            this.__llenarBox(filtrarArray(listado,subrubros), objSubrubros, 'eligio', subrubroSeleccionado);
        })
    }

    __filtrarBoxesParaRubros(consulta, objMarcas, objSubrubros, marcas, subrubros) {
        const url = new App().getUrlApi('boxesFiltrados');
        const parametros = `where=${consulta}`;
        console.log(url+'?'+parametros)
        new APIs().call(url, parametros, 'POST', xdatos => {
            ...
            objMarcas.innerHTML = '';
            this.__llenarBox(filtrarArray(listado,marcas), objMarcas, 'eligio', marcaSeleccionada);
            listado = filtrarNumerosArray(xdatos,'id_subrubro');
            let subrubroSeleccionado = objSubrubros.value;
            objSubrubros.innerHtml = '';
            this.__llenarBox(filtrarArray(listado,subrubros), objSubrubros, 'eligio', subrubroSeleccionado);
    ...
    __filtrarBoxesParaSubrubros(consulta, objMarcas, objRubros, marcas, rubros) {
        ...
        new APIs().call(url, parametros, 'POST', xdatos => {
            ...
            let rubroSeleccionado = objRubros.value;
            objRubros.innerHtml = '';
            this.__llenarBox(filtrarArray(listado,rubros), objRubros, 'eligio', rubroSeleccionado);
        })
    }
