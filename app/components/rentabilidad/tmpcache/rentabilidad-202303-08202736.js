class Rentabilidad extends ComponentManager {
    ...
    __llenarBoxFiltrado(query_selector, marcas, rubros, subrubros) {
        ...
        objMarcas.addEventListener('change', () => {
            let consulta = this.__crearConsultaParaFiltro(objMarcas, objRubros, objSubrubros);
            this.__filtrarBoxesParaMarcas(consulta, objRubros, objSubrubros, rubros, subrubros);
            
        })
        objRubros.addEventListener('change', () => {
            let consulta = this.__crearConsultaParaFiltro(objMarcas, objRubros, objSubrubros);
            this.__filtrarBoxesParaRubros(consulta, objRubros, objSubrubros, marcas, subrubros);
    ...
    __filtrarBoxesParaMarcas(consulta, objRubros, objSubrubros, rubros, subrubros) {
        ...
        new APIs().call(url, parametros, 'POST', xdatos => {
            ...
            objRubros.innerHTML = '';
            this.__llenarBox(filtrarArray(listado,rubros), objRubros);
            listado = filtrarNumerosArray(xdatos,'id_subrubro');
            objSubrubros.innerHtml = '';
            this.__llenarBox(filtrarArray(listado,subrubros), objSubrubros);
        })
    }
    ...
    __filtrarBoxesParaRubros(consulta, objMarcas, objSubrubros, marcas, subrubros) {
        ...
        new APIs().call(url, parametros, 'POST', xdatos => {
            ...
            objMarcas.innerHTML = '';
            this.__llenarBox(filtrarArray(listado,marcas), objMarcas);
            listado = filtrarNumerosArray(xdatos,'id_subrubro');
            objSubrubros.innerHtml = '';
            this.__llenarBox(filtrarArray(listado,subrubros), objSubrubros);
        })
    ...
    __crearConsultaParaFiltro(objMarcas, objRubros, objSubrubros) {
        let consulta = '';
...
function filtrarArray(arrayClave, arrayFiltrar) {
    ...
    for(let i=0;i<arrayClave.length;i++) {
        auxRubros.push(arrayFiltrar.filter(arr => arr.id == arrayClave[i])[0]);
    }
    return auxRubros;
}